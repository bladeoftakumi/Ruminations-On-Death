...
function Onboarding({ initial, onSubmit }) {
  ...
  steps.push({ id: "name" });
  steps.push({ id: "age" });
  steps.push({ id: "hasGoal" });
  steps.push({ id: "works" });
  if (state.works) {
    if (state.hasGoal) steps.push({ id: "goal" });
    ...
    <div className="onb">
      <div className="onb-frame">
        ...
        <footer className="onb-footer">
          ...
          {current.id !== "welcome" && current.id !== "works" && current.id !== "hasGoal" && current.id !== "goal" && current.id !== "submit" &&
          <button
            className="onb-next"
            onClick={next}
            disabled={!canAdvance}>
...
function OnbStep({ current, state, update, next, canAdvance }) {
  ...
  switch (current.id) {
    case "welcome":
      return (
        <div className="onb-content onb-content-welcome">
          <img className="onb-logo" src="assets/hourglass-logo.png" alt="" />
          <p className="onb-eyebrow">Before we begin</p>
          <h1 className="onb-title">A few small questions.</h1>
    ...
    case "hasGoal":
      return (
        <div className="onb-content">
          <p className="onb-eyebrow">Before your work</p>
          <h1 className="onb-title">Do you have a goal in life?</h1>
          <p className="onb-body">
            Something you are trying to build or become — a direction, however
            rough. Your answer shapes the questions that&nbsp;follow.
          </p>
          <OnbChoice
            value={state.hasGoal}
            options={[
            { value: true, label: "Yes, I have a goal" },
            { value: false, label: "Not yet" }]
            }
            onChange={(v) => {update({ hasGoal: v });setTimeout(next, 150);}} />
      ...
      return (
        <div className="onb-content">
          ...
          <OnbChoice
            ...
            options={[
            { value: true, label: "Yes — it is directly related to my goal" },
    ...
    case "screen":
      return (
        <div className="onb-content">
          ...
          <h1 className="onb-title">How many non-work hours a day are wasted scrolling?</h1>
          <p className="onb-body">
            Only your own time — leisure hours lost to a phone, scrolling, gaming
            or other small addictions. Time wasted during work doesn't count
